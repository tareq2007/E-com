...
def add_to_cart(request,pk):
    ...
    order_item = Cart.objects.get_or_create(item=item, user=request.user, purchaed=False)
    ...
    if order_qs.exists():
        ...
        order.save()
        order.orderitems.add(order_item[0])
        messages.warning(request,'This item was successfully added to your cart.')
        return redirect('App_Shop:home')
...
def cart_view(request):
    carts = Cart.objects.filter(user=request.user, purchaed=False)
...
def remove_from_cart(request,pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            order_item = Cart.objects.filter(item=item, user=request.user, purchaed=False)[0]
